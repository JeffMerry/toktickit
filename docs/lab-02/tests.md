...
2. **API / Integration Tests**: Backend REST HTTP endpoints (`POST /api/tickets`, `GET /api/tickets`, `GET /api/tickets/:id`), ownership isolation, attachment constraints, search, filtering, and pagination behavior.
3. **UI Component Tests**: React forms, field validation error placement, loading spinner states, badge rendering, and responsive ticket list presentation.
...
| **API-05** | API | AC-05, BR-11 | Upload 6th attachment to ticket with 5 active files | Status 400 Bad Request / Limit Error | `server/tests/api/attachments.api.test.ts` | Planned |
| **API-06** | API | AC-06, BR-12 | Soft-remove attachment with valid reason | Status 200; `isRemoved = true`; download blocked | `server/tests/api/attachments.api.test.ts` | Planned |
...
| **AC-03** (Ownership Protection) | `API-03` | Verified & Passing |
| **AC-04** (File Size & Type Validation) | `UNIT-01`, `UI-02` | Verified & Passing |
| **AC-05** (Max 5 Attachments Limit) | `API-05` | Planned |
| **AC-06** (Soft Removal with Reason) | `API-06` | Planned |
